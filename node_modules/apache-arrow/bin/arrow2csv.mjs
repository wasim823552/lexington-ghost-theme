...
import { RecordBatchReader, AsyncByteQueue, util } from '../Arrow.mjs';
import { parseCliArgs, formatUsage } from './cli.mjs';
import { parseArrowJSON } from '../util/json.mjs';
const { values: argv, positionals } = parseCliArgs(cliOpts(), process.argv.slice(2));
const files = argv.help
    ? []
    : [...((_a = argv.file) !== null && _a !== void 0 ? _a : []), ...positionals].filter(Boolean);
...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const source of sources) {
        ...
        try {
            for (var _e = true, _f = (e_1 = void 0, __asyncValues(recordBatchReaders(source))), _g; _g = yield _f.next(), _b = _g.done, !_b; _e = true) {
                _d = _g.value;
                _e = false;
                reader = _d;
                hasReaders = true;
                ...
                yield pipeTo(batches.pipe(toString), process.stdout, { end: false }).catch(() => state.closed = true); // Handle EPIPE errors
            }
        }
...
function recordBatchReaders(createSourceStream) {
    return __asyncGenerator(this, arguments, function* recordBatchReaders_1() {
        ...
        try {
            try {
                for (var _g = true, _h = __asyncValues(RecordBatchReader.readAll(stream)), _j; _j = yield __await(_h.next()), _a = _j.done, !_a; _g = true) {
                    _c = _j.value;
                    _g = false;
                    reader = _c;
        ...
        if (!readers) {
            ...
            try {
                try {
                    for (var _l = true, _m = __asyncValues(RecordBatchReader.readAll(parseArrowJSON(yield __await(json.toString())))), _o; _o = yield __await(_m.next()), _d = _o.done, !_d; _l = true) {
                        _f = _o.value;
                        _l = false;
                        reader = _f;
                        reader && (yield yield __await(reader));
