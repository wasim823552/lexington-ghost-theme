...
import { DataType, Dictionary, Utf8, LargeUtf8, Binary, LargeBinary, BinaryView, Utf8View, Decimal, FixedSizeBinary, List, LargeList, FixedSizeList, Map_, Struct, Union, Bool, Null, Int, Float, Date_, Time, Interval, Timestamp, Int32, Duration, } from '../../type.mjs';
/**
 * @ignore
 * @private
 **/
export class Message {
    ...
    static encode(message) {
        ...
        if (customMetadataOffset !== -1) {
            _Message.addCustomMetadata(b, customMetadataOffset);
        }
...
function decodeRecordBatch(batch, version = MetadataVersion.V5) {
    const recordBatch = new RecordBatch(batch.length(), decodeFieldNodes(batch), decodeBuffers(batch, version), decodeBodyCompression(batch.compression()), decodeVariadicBufferCounts(batch));
    return recordBatch;
}
/** @ignore */
