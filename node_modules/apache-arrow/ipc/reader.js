...
class RecordBatchReaderImpl {
    ...
    _loadRecordBatch(header, body, messageMetadata) {
        ...
        return new recordbatch_js_1.RecordBatch(this.schema, data, messageMetadata);
    }
    ...
    _loadVectors(header, body, types) {
        return new vectorloader_js_1.VectorLoader(body, header.nodes, header.buffers, this.dictionaries, this.schema.metadataVersion, header.variadicBufferCounts).visitMany(types);
    }
    _loadCompressedVectors(header, body, types) {
        return new vectorloader_js_1.CompressedVectorLoader(body, header.nodes, header.buffers, this.dictionaries, this.schema.metadataVersion, header.variadicBufferCounts).visitMany(types);
    }
    _decompressBuffers(header, body, codec) {
        ...
        for (const { offset, length } of header.buffers) {
            ...
            const decompressed = (uncompressedLenth === constants_js_1.LENGTH_NO_COMPRESSED_DATA)
                ? bytes
                : codec.decode(bytes);
...
class RecordBatchJSONReaderImpl extends RecordBatchStreamReaderImpl {
    ...
    _loadVectors(header, body, types) {
        return new vectorloader_js_1.JSONVectorLoader(body, header.nodes, header.buffers, this.dictionaries, this.schema.metadataVersion, header.variadicBufferCounts).visitMany(types);
    }
}
//
