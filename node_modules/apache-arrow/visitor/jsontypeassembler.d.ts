...
export declare class JSONTypeAssembler extends Visitor {
    ...
    visitLargeBinary<T extends type.LargeBinary>({ typeId }: T): {
        name: string;
    };
    visitBinaryView<T extends type.BinaryView>({ typeId }: T): {
        name: string;
    };
    ...
    visitLargeUtf8<T extends type.LargeUtf8>({ typeId }: T): {
        name: string;
    };
    visitUtf8View<T extends type.Utf8View>({ typeId }: T): {
        name: string;
    };
    ...
    visitDuration<T extends type.Duration>({ typeId, unit }: T): {
        name: string;
        unit: string;
    };
    ...
    visitLargeList<T extends type.LargeList>({ typeId }: T): {
        name: string;
    };
