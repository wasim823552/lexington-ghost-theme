...
class VectorLoader extends visitor_js_1.Visitor {
    constructor(bytes, nodes, buffers, dictionaries, metadataVersion = enum_js_1.MetadataVersion.V5, variadicBufferCounts = []) {
        super();
        this.nodesIndex = -1;
        this.buffersIndex = -1;
...
class JSONVectorLoader extends VectorLoader {
    ...
    readData(type, { offset } = this.nextBufferRange()) {
        ...
        else if (type_js_1.DataType.isBinaryView(type)) {
            return binaryViewDataFromJSON(sources[offset]);
        }
        else if (type_js_1.DataType.isUtf8View(type)) {
        ...
        else if (type_js_1.DataType.isInterval(type)) {
            switch (type.unit) {
                case enum_js_1.IntervalUnit.DAY_TIME:
                    return (0, interval_js_1.toIntervalDayTimeInt32Array)(sources[offset]);
                case enum_js_1.IntervalUnit.MONTH_DAY_NANO:
                    return (0, interval_js_1.toIntervalMonthDayNanoInt32Array)(sources[offset]);
                default:
                    break;
            }
        }
